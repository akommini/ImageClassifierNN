...
def partial_derivatives(x, y, W, V, b, c):
    ...
    h = sig(b + W @ x)
    h_bar = sigp(b + W @ x)
    #error=np.divide(np.subtract(l,y),LA.norm(np.subtract(l,y)))
    error=np.divide(np.subtract(l,l[y]),LA.norm(np.subtract(l,l[y])))
    dLdc = np.ones(c.shape)
    dLdV = np.ones(V.shape)
    dLdb = np.ones(b.shape)
    dLdW = np.ones(W.shape)
    dLdc = np.divide(np.exp(l),np.sum(np.exp(l)))
    dLdV = np.matmul(dLdc,h.transpose())
    dLdb = np.multiply(h_bar,np.matmul(V.transpose(),dLdc))
    dLdW = np.multiply(h_bar,np.matmul(np.matmul(V.transpose(),dLdc),x.transpose()))
