...
def mean_zero_one_loss(weights, x, y_integers, unflatten):
	...
	return(out,(np.mean(pred != y_integers)))
...
for x in range(nEpochs):
    ...
    log_loss.append(returned_values[0])
    Train_error.append(returned_values[1])
...
smooth_grad = (1 - momentum) * smooth_grad + momentum * weight_gradients
weights = weights - epsilon * smooth_grad
out_final,Loss=mean_zero_one_loss(weights, train_x, train_y_integers, unflatten)

print('Train accuracy =', 1-Loss)

# Output file location
file_name = '../Predictions/best_NN.csv'
# Writing output in Kaggle format
print('Writing output to ', file_name)
kaggle.kaggleize(out_final, file_name)